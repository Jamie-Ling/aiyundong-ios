...
- (void)commonSetup
{
    ...
    for (int i = 0; i < self.count; i++)
    {
        ...
        item.center = CGPointMake(self.center.x + 10, self.center.y + 10);
    ...
    for (int i = 0; i < self.count; i++)
    {
        UISnapBehavior *snap = [[UISnapBehavior alloc] initWithItem:self.items[i] snapToPoint:CGPointMake(self.center.x + 10, self.center.y + 10)];
...
{
    CGFloat firstAngle = M_PI * 0.75 + (M_PI_2 - self.angle) + index * self.angle;
...
{
    UISnapBehavior *snap = [[UISnapBehavior alloc] initWithItem:self.items[index] snapToPoint:CGPointMake(self.center.x + 10, self.center.y + 10)];
    snap.damping = self.sphereDamping;
