...
#define vTableViewMoveLeftX 20  //tableview向左移20
...
@interface UserInfoViewController ()<UITableViewDataSource, UITableViewDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MSDatePickerDelegate>
{
...
{
    SignatureViewController *signatureView = [[SignatureViewController alloc] init];
    signatureView.signatureString = [_userInfoDictionary objectForKey:kUserInfoOfDeclarationKey];
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0)
    {
        FlatRoundedButton *userImageButton = [[ObjectCTools shared] getARoundedButtonWithSize:vOneCellHeight - 13 withImageName:[_userInfoDictionary objectForKey:kUserInfoOfHeadPhotoKey]];  //实际应该是一个url从服务器获取图片显示
...
- (void) checkOneDate:(NSDate *)theDate
{
    ...
    NSString *getDateString = [dateFormatter stringFromDate:theDate];
    
    if ([getDateString isEqualToString:[_userInfoDictionary objectForKey:kUserInfoOfAgeKey]])
    {
        //相同，没有修改
        return;
    }
